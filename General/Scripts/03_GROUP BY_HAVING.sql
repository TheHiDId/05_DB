...
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE AS "부서코드", SUM(SALARY) AS "급여 합", FLOOR(AVG(SALARY)) AS "평균", COUNT(*) AS "인원 수"
FROM EMPLOYEE 
GROUP BY DEPT_CODE 
ORDER BY DEPT_CODE ASC;
...
-- 인원수로 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남자', '2', '여자') AS "성별", FLOOR(AVG(SALARY)) AS "급여 평균", SUM(SALARY) AS "급여 합계", COUNT(*) AS "인원 수"
FROM EMPLOYEE 
...
FROM EMPLOYEE 
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE 그룹 내에 JOB_CODE로 다시 그룹을 만듦
ORDER BY DEPT_CODE ASC;
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
-- 개인 급여가 350만 이상인 사람들만 모아서 부서별로 그룹을 나누고 평균을 구함 따라서 요구사항과 다름
...
ORDER BY DEPT_CODE ASC;
-- 부서 그룹별로 급여 평균을 구했을 때 급여 평균이 350만 이상인 부서만 조회
...
-- 부서코드가 D5인 사원의 이름, 부서코드 조회
SELECT EMP_NAME, DEPT_CODE, JOB_CODE 
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D5'

UNION -- 합집합의 결과는 OR 연산과 같음

-- 직급코드가 J5인 사원의 이름, 부서코드, 직급코드 조회
SELECT EMP_NAME, DEPT_CODE, JOB_CODE 
FROM EMPLOYEE 
WHERE JOB_CODE = 'J7';
