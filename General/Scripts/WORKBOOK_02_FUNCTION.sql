...
-- 나이는 '만'으로 계산한다.)
SELECT PROFESSOR_NAME AS "교수이름", FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('19'||SUBSTR(PROFESSOR_SSN, 1, 6), 'YYYY-MM-DD')) / 12) AS "나이"
FROM TB_PROFESSOR
ORDER BY MONTHS_BETWEEN(SYSDATE, TO_DATE('19'||SUBSTR(PROFESSOR_SSN, 1, 6), 'YYYY-MM-DD')) / 12 ASC;
...
SELECT STUDENT_NO, STUDENT_NAME, FLOOR(MONTHS_BETWEEN(ENTRANCE_DATE, TO_DATE('19'||SUBSTR(STUDENT_SSN, 1, 6), 'YYYY-MM-DD')) / 12) AS "입학 나이"
FROM TB_STUDENT
...
SELECT D.DEPARTMENT_NO AS "학과코드", COUNT(DECODE(S.ABSENCE_YN, 'Y', '휴학생')) AS "휴학생 수"
